...
public interface IRenderer {
    ...
    void fill(MatrixStack matrixStack, int minX, int minY, int maxX, int maxY, int color);
    void hLine(MatrixStack matrixStack, int minX, int maxX, int y, int color);
    void vLine(MatrixStack matrixStack, int x, int minY, int maxY, int color);
    int drawString(MatrixStack matrixStack, FontRenderer fontRenderer, String font, int text, int x, int y);
    int drawString(MatrixStack matrixStack, FontRenderer fontRenderer, IReorderingProcessor processor, int text, int x, int y);
    void fillGradient(MatrixStack matrixStack, int x1, int y1, int x2, int y2, int colorFrom, int colorTo);
    void drawCenteredString(MatrixStack matrixStack, FontRenderer fontRenderer, String font, int text, int x, int y);
    void drawCenteredString(MatrixStack matrixStack, FontRenderer fontRenderer, ITextComponent font, int text, int x, int y);
    int drawString(MatrixStack matrixStack, FontRenderer fontRenderer, ITextComponent font, int text, int x, int y);
    void blit(MatrixStack matrixStack, int x, int y, int width, int height, float uOffset, float vOffset, int uWidth, int vHeight, int textureWidth, int textureHeight);
    void blit(MatrixStack matrixStack, int x, int y, float uOffset, float vOffset, int width, int height, int textureWidth, int textureHeight);
...
//            RenderSystem.depthMask(true);
        ...
        private void fillGradient(Matrix4f matrix, BufferBuilder builder, int x1, int y1, int x2, int y2, int z, int colorA, int colorB) {
        ...
        public int drawString(MatrixStack matrixStack, FontRenderer fontRenderer, String font, int text, int x, int y) {
            return fontRenderer.drawStringWithShadow(matrixStack, font, (float)text, (float)x, y);
        }

        @Override
        public int drawString(MatrixStack matrixStack, FontRenderer fontRenderer, IReorderingProcessor processor, int x, int y, int color) {
            return fontRenderer.drawTextWithShadow(matrixStack, processor, (float)x, (float)y, color);
        }

        public int drawString(MatrixStack matrixStack, FontRenderer fontRenderer, ITextComponent font, int text, int x, int y) {
            return fontRenderer.drawTextWithShadow(matrixStack, font, (float)text, (float)x, y);
        }
