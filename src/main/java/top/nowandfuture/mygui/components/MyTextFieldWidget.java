...
class MyTextFieldWidget extends TextFieldWidget {
    ...
    private Color selectionColor = new Color(0, 0, 255, 128);
    ...
    public MyTextFieldWidget(FontRenderer fontRenderer, int x, int y, int width, int height, ITextComponent title) {
        this(fontRenderer, x, y, width, height, null, title);
    }

    public MyTextFieldWidget(FontRenderer fontRenderer, int x, int y, int width, int height, @Nullable TextFieldWidget inputWidget, ITextComponent title) {
        super(fontRenderer, x, y, width, height, title);
        this.fontRenderer = fontRenderer;
        if (inputWidget != null) {
            this.setText(inputWidget.getText());
        }
    }
    ...
    public void renderWidget(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.getVisible()) {
            if (this.getEnableBackgroundDrawing()) {
                ...
                GUIRenderer.getInstance().fill(matrixStack, this.x - 1, this.y - 1, this.x + this.width + 1, this.y + this.height + 1, i);
                GUIRenderer.getInstance().fill(matrixStack, this.x, this.y, this.x + this.width, this.y + this.height, -16777216);
            }
            ...
            if (!s.isEmpty()) {
                ...
                if (drawShadow)
                    j1 = this.fontRenderer.drawTextWithShadow(matrixStack, this.textFormatter.apply(s1, this.lineScrollOffset), (float) l, (float) i1, i2);
                else
                    j1 = GUIRenderer.getInstance().drawString(matrixStack, this.fontRenderer, this.textFormatter.apply(s1, this.lineScrollOffset), l, i1, i2);
            ...
            if (!s.isEmpty() && flag && j < s.length()) {
                if (drawShadow)
                    this.fontRenderer.drawTextWithShadow(matrixStack, this.textFormatter.apply(s.substring(j), this.cursorPosition), (float) j1, (float) i1, i2);
                else
                    GUIRenderer.getInstance().drawString(matrixStack, this.fontRenderer, this.textFormatter.apply(s.substring(j), this.cursorPosition), j1, i1, i2);
            ...
            if (!flag2 && this.suggestion != null) {
                if (drawShadow)
                    ...
                    GUIRenderer.getInstance().drawString(matrixStack, this.fontRenderer, this.suggestion,  (k1 - 1), i1, -8355712);
            }

            if (flag1) {
                if (flag2) {
                    GUIRenderer.getInstance().fill(matrixStack, k1, i1 - 1, k1 + 1, i1 + 1 + 9, cursorColor);
                } else {
                    if (drawShadow)
     ...
     */
    private void drawSelectionBox(MatrixStack stack, int startX, int startY, int endX, int endY) {
        if (startX < endX) {
            int i = startX;
            startX = endX;
            endX = i;
...
//        RenderSystem.color4f(selectionColor.getRed(), selectionColor.getGreen(), selectionColor.getBlue(), selectionColor.getAlpha());
...
//        RenderSystem.enableTexture();
        ...
        RenderSystem.enableColorLogicOp();
        RenderSystem.logicOp(GlStateManager.LogicOp.OR_REVERSE);
        GUIRenderer.getInstance().fill(stack, startX, startY, endX, endY, selectionColor.colorInt());
        RenderSystem.disableColorLogicOp();
        RenderSystem.enableTexture();
    }
    ...
    public void setSelectionPos(int position) {
        ...
        if (this.fontRenderer != null) {
            ...
            if (this.selectionEnd == this.lineScrollOffset) {
                this.lineScrollOffset -= this.fontRenderer.getLineScrollOffset(this.text, j, true).length();
            }
