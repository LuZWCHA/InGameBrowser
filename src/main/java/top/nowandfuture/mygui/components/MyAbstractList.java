...
public abstract class MyAbstractList<T extends MyAbstractList.ViewHolder> extends FrameLayout {
    ...
    protected void onDraw(MatrixStack stack, int mouseX, int mouseY, float partialTicks) {
        ...
        for (int index = startIndex; index < Math.min(startIndex + computeItemNum(), getAdapter().getSize()); index++) {
            ...
            GlStateManager.translatef(0, getAdapter().getHeight() + offsetY, 0);
            drawSplitLine();
    ...
    private void drawSliderIn(MatrixStack stack) {
        ...
        int top = sliderPosY;
        sliderSize = size;
        drawSlider(stack, left, top, size);
    }

    protected void drawSlider(MatrixStack stack, int left, int top, int size) {
        drawRect(stack, left, top, getWidth(), top + size, colorInt(200, 200, 200, 255));
    }
